...
export default function AdminDashboard({
  ...
  // Passcode authentication is non-persistent (1x session only)
  ...
  const settingsLogoFileRef = useRef<HTMLInputElement>(null);
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (passcode === 'adminbilal123') {
      const mySid = sessionStorage.getItem('my_admin_sid');
      const activeSid = localStorage.getItem('active_admin_sid');
      const lastPing = localStorage.getItem('active_admin_ping');
      const now = Date.now();

      // Exclusivity session validation
      if (activeSid && mySid && activeSid !== mySid && lastPing && now - parseInt(lastPing, 10) < 42000) {
        setPasscodeError('Akses Ditolak: Sesi admin sedang aktif di browser/perangkat lain.');
        return;
      }

      // Claim the lock immediately
      if (mySid) {
        localStorage.setItem('active_admin_sid', mySid);
  ...
  const handleLogoFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        showToast('Foto logo berhasil dimuat!');
      };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                ...
                <form onSubmit={handleSaveSettings} className="space-y-4">
                  ...
                  {/* Store Logo Photo Upload (Logo WEB/APK bisa dengan Foto) */}
                  <div className="p-3.5 rounded-xl bg-white/[0.01] border border-white/5 space-y-3">
                    <div className="text-xs font-semibold text-zinc-300 font-display flex items-center gap-1.5">
                      ...
                      Logo Website / APK (Foto Kustom)
                    </div>
                    <div className="flex flex-col sm:flex-row gap-4 items-center">
                      <div className="w-12 h-12 border border-white/10 rounded-full overflow-hidden bg-black flex items-center justify-center shrink-0">
                        {storeLogo ? (
                          <img src={storeLogo} alt="Store Logo" className="w-full h-full object-cover" />
                        ) : (
                          <span className="text-[10px] text-zinc-600 font-mono font-bold">LOGO</span>
                        )}
                      </div>
                      <div className="flex-1 space-y-2 w-full">
                        <button
                          type="button"
                          onClick={() => settingsLogoFileRef.current?.click()}
                        ...
                        >
                          <Upload size={14} />
                          Pilih Foto Logo Toko
                        </button>
                        <input
                          type="file"
                          ref={settingsLogoFileRef}
                        ...
                        {storeLogo && (
                          <button
                            ...
                            className="text-[10px] text-red-400 hover:text-red-300 transition block text-left"
                          >
                            Hapus Logo (Kembali ke Default L)
                          </button>
                        )}
                      </div>
                    </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="px-3 py-1.5 text-2xs rounded-xl glass-input text-white w-40 sm:w-48"
                  />
                  <div className="text-xs text-zinc-400 font-mono shrink-0">Total: {products.length}</div>
                </div>
              </div>
